...
instrument = minimalmodbus.Instrument("/dev/plc", servo_x_addr) 
...
def callback3(cmd_vel):
    ...
    var_write = instrument.write_register(8182, abs(speed_send_right))           # 1st motor pulse freq.  
    ...
    var_write = instrument.write_register(8185, abs(speed_send_left))          # 2nd motor pulse freq.
...
def speed_control_rev():
    ...
    rospy.init_node('act',anonymous = True)                                # Node initiation. Node name: actuator
