...
AREA_THRESHOLD = 750 #Area threshold for color detection
...
pan_angle = 30   #Set initial angles for servos (0-180)

tilt_angle = 30



pan_gain = 0.05   #Set gains for proportional control
...
#Convert BGR to HSV and define lower,upper bounds for color mask
...
#Enter HSV values directly after obtaining them from color_mask.py
...
def PanAngle(x,gain):
    ...
    if x < CENTER[0] - 10: #Threshold for center

        pan_angle -= int(gain*np.abs(CENTER[0]-x)) #Control speed of servo panning proportional to error
    ...
    if x > CENTER[0] + 10:
        ...
        pan_angle += int(gain*np.abs(CENTER[0]-x))       
...
def TiltAngle(y,gain):

    global tilt_angle

    if y < CENTER[1] - 10:

        tilt_angle -= int(gain*np.abs(CENTER[1]-y))
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    pan_angle = PanAngle(x_center,pan_gain)

    tilt_angle = TiltAngle(y_center,tilt_gain)
